...
const encode = (str) => encodeURIComponent("cute cartoon " + str);
const getImg = (prompt, w=400, h=300) => `https://image.pollinations.ai/prompt/${encode(prompt)}?width=${w}&height=${h}&nologo=true`;
...
for (const l of lettersRaw) {
  let words = l.w.map(word => `{ text: "${word}", image: "${getImg(word)}" }`).join(",\n      ");
  let sentences = l.s.map(sentence => `{ text: "${sentence}", image: "${getImg(sentence)}" }`).join(",\n      ");
  
  let mainImage = l.l === "A" ? "/src/assets/apple.png" : l.l === "B" ? "/src/assets/bear.png" : getImg(`letter ${l.l} character`, 400, 400);
...
for (const s of sentencesRaw) {
  let items = s.i.map(item => `{ text: "${item}", image: "${getImg(item)}" }`).join(",\n      ");
  sentencesDataOut += `  { \n    id: "${s.id}", \n    title: "${s.t}", \n    items: [\n      ${items}\n    ] \n  },\n`;
}
sentencesDataOut += "];\n";

fs.writeFileSync('src/data.js', lettersDataOut + sentencesDataOut);
console.log("data.js generated with images");
